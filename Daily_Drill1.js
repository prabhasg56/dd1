...
//Q1. Find all Web Developers

const output1=data.filter((d)=>
    ((d.job==='Web Developer II')||(d.job==='Web Developer III'))       
)
console.log(output1)


//Q2. Convert all the salary values into proper numbers instead of strings 

for(let i=0;i<data.length;i++){
    let d1=data[i].salary.replace(/[$,]/g, "");
    console.log(parseFloat(d1));
}



//Q3. Assume that each salary amount is a factor of 10000 and correct it but add it as a new key (corrected_salary or something)

//Q4. Find the sum of all salaries 
let sum=0;
for(let i=0;i<data.length;i++){
    let d1=data[i].salary.replace(/[$,]/g, "");
    sum +=(parseFloat(d1));
...
//Q5. Find the sum of all salaries based on country using only HOF method



//Q6. Find the average salary of based on country using only HOF method
